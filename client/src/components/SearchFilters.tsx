    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between lg:hidden">
        <button onClick={() => setMobileOpen(!mobileOpen)} className="portal-btn-secondary py-2 text-xs flex items-center gap-2">
          <svg className="h-4 w-4 text-[#024738]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" />
          </svg>
          Filters & Search
          {hasActiveFilters && <span className="flex h-2 w-2 rounded-full bg-[#C0F762]" />}
        </button>
        {hasActiveFilters && (
          <button onClick={onReset} className="text-xs text-red-600 font-bold hover:underline">
            Reset Filters
          </button>
        )}
      </div>
      ...
      {/* Main filter panel */}
      <div className={`portal-card p-5 space-y-4 ${mobileOpen ? 'block' : 'hidden lg:block'}`}>
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
          {/* Search Input */}
          <div className="md:col-span-4 relative">
            <input
              type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
              placeholder="Search make, model, VIN..." className="portal-input pl-10 text-xs"
            />
            <svg className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-[#47695F]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="md:col-span-2">
            <select value={selectedMake} onChange={(e) => setSelectedMake(e.target.value)} className="portal-input text-xs">
          ...
          {/* Stock Select */}
          <div className="md:col-span-2">
            <select value={stockStatus} onChange={(e) => setStockStatus(e.target.value)} className="portal-input text-xs">
          ...
          <div className="md:col-span-2 flex items-center gap-1.5">
            ...
            <span className="text-[#47695F] text-xs shrink-0">–</span>
          ...
          <div className="md:col-span-2 flex items-center gap-2">
            ...
            {hasActiveFilters && (
              <button onClick={onReset} title="Reset filters"
                className="portal-btn-secondary py-2 px-2.5 text-xs text-red-600 hover:bg-red-50 shrink-0">
                ✕
              </button>
            )}
          </div>
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-0.5">
          <span className="text-[11px] font-bold uppercase tracking-wider text-[#47695F] shrink-0">Category:</span>
          {categories.map((cat) => (
            <button
              key={cat}
              ...
              className={`rounded-full px-4 py-1.5 text-xs font-bold transition-all shrink-0 ${
                selectedCategory === cat
                  ? 'bg-[#024738] text-[#C0F762] shadow-sm'
                  : 'bg-[#E8F7ED] text-[#024738] hover:bg-[#D1EFE0]'
              }`}
            >
              {cat}
            </button>
