    ...
    <header className="sticky top-0 z-40 w-full border-b border-slate-200 bg-white shadow-sm">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        ...
        <Link to="/" className="flex items-center gap-2.5 shrink-0">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#6D5DFB] shadow-md shadow-[#6D5DFB]/20">
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-all duration-150 ${
                activeTab === tab.id
                  ? 'bg-[#6D5DFB]/10 text-[#6D5DFB] font-semibold'
                  : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100'
              }`}
            >
              <span className={activeTab === tab.id ? 'text-[#6D5DFB]' : 'text-slate-400'}>
                {tab.icon}
              </span>
              {tab.label}
            </button>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative hidden sm:block">
            ...
            {globalSearch && (
              <button onClick={() => setGlobalSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 transition-colors">
          ...
          {isAdmin && onOpenAddModal && (
            <button onClick={onOpenAddModal} className="portal-btn-primary py-2 px-3.5 text-sm hidden sm:flex">
          ...
          {user && (
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2.5 rounded-xl border border-slate-200 bg-white px-2.5 py-1.5 hover:border-slate-300 hover:bg-slate-50 transition-all shadow-sm"
              >
                <div className={`flex h-7 w-7 items-center justify-center rounded-lg text-xs font-bold text-white uppercase ${isAdmin ? 'bg-amber-500' : 'bg-[#6D5DFB]'}`}>
                  {initials}
                </div>
                <div className="hidden sm:flex flex-col text-left">
                  <span className="text-xs font-semibold text-slate-700 leading-tight truncate max-w-[100px]">
                    {user.email.split('@')[0]}
                  </span>
                  <span className={`text-[9px] font-bold uppercase tracking-widest leading-tight ${isAdmin ? 'text-amber-500' : 'text-[#6D5DFB]'}`}>
                    {user.role}
                  </span>
                </div>
              ...
              {showProfileMenu && (
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-52 rounded-2xl bg-white border border-slate-200 p-1.5 shadow-xl shadow-slate-200/80 z-50 animate-slide-up">
                    <div className="px-3 py-2.5 border-b border-slate-100 mb-1">
                      <p className="text-xs font-semibold text-slate-800 truncate">{user.email}</p>
                      <p className="text-[10px] text-slate-400 mt-0.5">
                        Role: <span className={`font-bold ${isAdmin ? 'text-amber-500' : 'text-[#6D5DFB]'}`}>{user.role}</span>
                      </p>
                    </div>
                    <button
                      onClick={() => { setShowProfileMenu(false); logout(); }}
                      className="w-full flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-sm text-red-500 hover:bg-red-50 hover:text-red-600 transition-colors"
                    >
                      <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button
            ...
            className="lg:hidden p-2 text-slate-500 hover:text-slate-900 hover:bg-slate-100 rounded-xl border border-slate-200 transition-colors"
          >
            {mobileMenuOpen ? (
      ...
      {/* Mobile menu */}
      {mobileMenuOpen && (
        <div className="lg:hidden border-t border-slate-100 bg-white px-4 pb-4 pt-3 animate-slide-up">
          <div className="relative mb-3 sm:hidden">
            <input type="text" value={globalSearch} onChange={(e) => setGlobalSearch(e.target.value)}
              placeholder="Search vehicles…" className="portal-input py-2.5 pl-9 text-sm" />
            <svg className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
          </div>
            ...
            {tabs.map((tab) => (
              <button key={tab.id} onClick={() => { setActiveTab(tab.id); setMobileMenuOpen(false); }}
                className={`w-full flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition-all ${
                  activeTab === tab.id ? 'bg-[#6D5DFB]/10 text-[#6D5DFB]' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50'
                }`}>
                <span className={activeTab === tab.id ? 'text-[#6D5DFB]' : 'text-slate-400'}>{tab.icon}</span>
                {tab.label}
              </button>
            ))}
            {isAdmin && onOpenAddModal && (
              <button onClick={() => { onOpenAddModal(); setMobileMenuOpen(false); }}
