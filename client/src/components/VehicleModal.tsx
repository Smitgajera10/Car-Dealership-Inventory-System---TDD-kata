...
export function VehicleModal({ isOpen, onClose, onSubmit, initialData, isLoading = false }: VehicleModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-fade-in" onClick={onClose}>
      <div className="portal-card max-w-lg w-full p-6 shadow-2xl bg-white border border-[#D1EFE0] rounded-3xl animate-scale-in max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>

        {/* Header */}
        <div className="flex items-center justify-between border-b border-[#E8F7ED] pb-4 mb-5">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#E8F7ED] border border-[#D1EFE0]">
            ...
            <div>
              <h3 className="text-xl font-serif font-bold text-[#024738]">{isEditing ? 'Edit Vehicle Spec' : 'Add New Vehicle'}</h3>
              <p className="text-xs text-[#47695F] font-medium">Automotive inventory management</p>
            </div>
          </div>
          <button onClick={onClose} className="text-[#47695F] hover:text-[#024738] transition-colors p-1 rounded-full hover:bg-[#E8F7ED]">
            ✕
          </button>
        </div>

        {error && (
          <div className="mb-4 flex items-center gap-2 rounded-2xl border border-red-200 bg-red-50 p-3 text-xs text-red-600 font-bold">
            <span>⚠️</span>
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Image preview */}
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-[#47695F] mb-1.5">Image URL</label>
            <div className="relative h-32 w-full bg-[#F7FDF9] rounded-2xl border border-[#D1EFE0] overflow-hidden flex items-center justify-center mb-2">
              {imageUrl && !imagePreviewError ? (
                ...
                <div className="text-center text-xs text-[#5E7E75] font-medium">
                  <svg className="h-8 w-8 mx-auto mb-1 text-[#D1EFE0]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                  </svg>
                  {imageUrl ? 'Invalid image URL' : 'Enter URL below for live preview'}
                </div>
              )}
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-[#47695F] mb-1">Make *</label>
              <input type="text" required value={make} onChange={(e) => setMake(e.target.value)} placeholder="e.g. Porsche" className="portal-input text-xs" />
            </div>
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-[#47695F] mb-1">Model *</label>
              <input type="text" required value={model} onChange={(e) => setModel(e.target.value)} placeholder="e.g. 911 GT3" className="portal-input text-xs" />
            </div>
          </div>

          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-[#47695F] mb-1">Category *</label>
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-[#47695F] mb-1">Price ($) *</label>
              ...
              <span className="text-[10px] text-[#059669] font-bold mt-0.5 block">{formattedPricePreview}</span>
            </div>
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-[#47695F] mb-1">Quantity *</label>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-[#E8F7ED]">
